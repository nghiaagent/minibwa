...
It also natively supports directional bisulfite sequencing data to higher mapping accuracy.
\vspace{0.5em}\\
...
with an asymmetric scoring matrix that penalizes a T-to-C mismatch but not C-to-T mismatch, which is in principle more informative.
In practice, however, the penalty applies only at unmethylated C positions on the converted strand,
and the dominant source of read-reference disagreement on directional BS-seq data is genuine C-to-T conversion rather than spurious T-to-C mismatch,
so the theoretical advantage may not translate into measurable accuracy gains (see Section~\ref{sec:eval-sim}).
